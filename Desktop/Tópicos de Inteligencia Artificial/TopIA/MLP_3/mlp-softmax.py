...
def softmax(x):
    exp_x = np.exp(x - np.max(x))  # Quitamos el valor máximo para mayor estabilidad numérica
    return exp_x / np.sum(exp_x, axis=1, keepdims=True)

# Pérdida de entropía cruzada con regularización L2
def cross_entropy_loss(y_true, y_pred, W1, W2, lambda_=0.01):
    n_samples = y_true.shape[0]
    y_pred_clipped = np.clip(y_pred, 1e-9, 1 - 1e-9)
    cross_entropy = -np.sum(y_true * np.log(y_pred_clipped)) / n_samples
    l2_reg = (np.sum(np.square(W1)) + np.sum(np.square(W2))) * (lambda_ / (2 * n_samples))
...
class MLP_Softmax:
    def __init__(self, input_size, hidden_size, output_size):
        # Xavier Initialization para capa oculta, He Initialization para ReLU
    ...
    def backward(self, X, y_true, learning_rate=0.01, lambda_=0.01):
        # Propagación hacia atrás (backpropagation)
        n_samples = X.shape[0]

        # Derivada de la capa de salida
        dz2 = cross_entropy_derivative(y_true, self.a2)
        dW2 = np.dot(self.a1.T, dz2) / n_samples + (lambda_ * self.W2 / n_samples)
        db2 = np.sum(dz2, axis=0, keepdims=True) / n_samples

        # Derivadas de la capa oculta (ReLU)
        dz1 = np.dot(dz2, self.W2.T) * relu_derivative(self.z1)
        dW1 = np.dot(X.T, dz1) / n_samples + (lambda_ * self.W1 / n_samples)
    ...
    def train(self, X, y_true, epochs=1000, learning_rate=0.01, lambda_=0.01):
        for epoch in range(epochs):
            # Forward pass
            y_pred = self.forward(X)

            # Calculamos la pérdida con regularización L2
            loss = cross_entropy_loss(y_true, y_pred, self.W1, self.W2, lambda_)

            # Backward pass
            self.backward(X, y_true, learning_rate, lambda_)

            if epoch % 100 == 0:
                print(f'Epoch {epoch}, Loss: {loss}')

# Inicializar y entrenar el MLP con regularización
mlp = MLP_Softmax(input_size=784, hidden_size=128, output_size=10)  # 784 entradas, 128 neuronas ocultas, 10 salidas
mlp.train(X, y_one_hot, epochs=1000, learning_rate=0.1, lambda_=0.01)
