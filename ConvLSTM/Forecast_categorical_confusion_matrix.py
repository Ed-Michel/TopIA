...
def evaluation2(args):
    test, naive, l_clas = args
    f = np.zeros((l_clas, l_clas), dtype=np.uint64)
    n = np.zeros((l_clas, l_clas), dtype=np.uint64)
    for k in range(test.shape[0]):
        for i in range(test.shape[1]):
...
def multi_process_evaluation2(test, naive, l_clas):
    cm_f = np.zeros((l_clas, l_clas), dtype=np.uint64)
    cm_n = np.zeros((l_clas, l_clas), dtype=np.uint64)
    print(cm_f)
    res_lists = zip(test, naive)
    args = [(t, n, l_clas) for (t, n) in res_lists]
    num_cores = multiprocessing.cpu_count()
    with ProcessPoolExecutor(max_workers=num_cores-1) as pool:
        with tqdm(total= len(test)) as progress:
            futures = []
            ...
            for eval in args:
                future = pool.submit(evaluation2, eval)
                future.add_done_callback(lambda p: progress.update())
                futures.append(future)
...
def main():
    ...
    for ele in naive:
        ...
        for _ in range(h):
            aux.append(ele[-1])
        new_naive.append(np.array(aux))
        
    #naive = naive[:, -h:]
    naive = np.array(new_naive)
    ...
    cm_f, cm_n = multi_process_evaluation2(y_test, naive, l_clas)
